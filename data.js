...
const VILLAIN_NAMES = {
  medieval:   ["흑염의 군주 발더로스","폐허왕 그림하트","저주받은 기사 모르덴","마룡 카르가스","어둠의 대마법사 넬타르"],
  wuxia:      ["혈마교주 혈전군","천마신교 교주","독왕 사마혼","마검 패천","흑풍채 채주"],
  cyberpunk:  ["기업 총수 카이저","AI 군주 옴니코어","넷 황제 글리치","사이코 보스 바이퍼","크롬 군벌 레이븐"],
  apocalypse: ["황무지 군벌 스컬","변이 군주 모르타","약탈왕 블러드팽","방사능 예언자 글로우","폐허의 왕 카리온"],
  mythology:  ["타락한 신 케이오스","거인왕 타이폰","명계의 군주 하데르","파멸의 화신 아바돈","폭풍의 신 테무스"],
  steampunk:  ["강철 남작 코그스워스","증기 황제 발브라이트","기계 군주 오토마톤","에테르 거물 볼타","톱니 총독 기어하트"],
};

const assignVillainName = (scenario) => {
  const key = scenario && (
    scenario.includes("중세") ? "medieval" :
    scenario.includes("무협") ? "wuxia" :
    scenario.includes("사이버") ? "cyberpunk" :
    scenario.includes("아포칼") ? "apocalypse" :
    scenario.includes("신화") ? "mythology" :
    scenario.includes("스팀") ? "steampunk" : "medieval"
  );
  const pool = VILLAIN_NAMES[key] || VILLAIN_NAMES.medieval;
  return pool[Math.floor(Math.random() * pool.length)];
};

const tickVillainGrowth = (playerAction = "none", scenario = "") => {
  ...
  // 위협도 25 이상이 되면 악당 이름 부여 (medium 진입 시 정체 드러남)
  if (villain.power >= 25 && !villain.name) {
    villain.name = assignVillainName(scenario);
  }
