...
const LOCAL_ITEM_FORM_BANK = {
  weapon:    [['검','⚔️'],['장검','⚔️'],['대검','⚔️'],['세검','⚔️'],
              ...
              ['철퇴','🔨'],['전투망치','🔨'],
              ['지팡이','🪄'],['마법봉','🪄']],
  subweapon: [['방패','🛡️'],['버클러','🛡️'],['토템','🗿'],['부적','📜'],
              ['오브','🔮'],['수정구','🔮'],['마도서','📖'],['깃발','🚩']],
  helmet:    [['투구','⛑️'],['헬름','⛑️'],['후드','🥷'],['두건','🥷'],
              ['왕관','👑'],['티아라','👑'],['가면','🎭'],['면갑','🎭']],
  armor:     [['갑옷','🥋'],['판금갑옷','🥋'],['사슬갑옷','🥋'],
              ['로브','🥼'],['드레스','🥼'],
              ['조끼','🦺'],['흉갑','🛡️'],['가슴받이','🛡️']],
  gloves:    [['장갑','🧤'],['건틀렛','🧤'],['손목보호대','🧤'],['아대','🧤'],['핸드가드','🧤']],
  boots:     [['장화','👢'],['부츠','🥾'],['워커','🥾'],['신발','👞'],['각반','🥾']],
  cloak:     [['망토','🧣'],['케이프','🧣'],['외투','🧥'],['코트','🧥'],['숄','🧣']],
  necklace:  [['목걸이','📿'],['펜던트','📿'],['초커','📿'],['목장식','📿'],['부적 목걸이','📿']],
  ring1:     [['반지','💍'],['인장반지','💍'],['보석반지','💍'],['각인반지','💍']],
  ring2:     [['반지','💍'],['인장반지','💍'],['보석반지','💍'],['각인반지','💍']],
  belt:      [['허리띠','🪢'],['벨트','🪢'],['요대','🪢'],['가죽벨트','🪢'],['금속벨트','⛓️']],
  trinket:   [['부적','📜'],['유물','🏺'],['장신구','🔮'],['목각인형','🪆'],
              ['오래된 동전','🪙'],['낡은 열쇠','🗝️'],['수호석','🔮'],['인장','📜']],
};
...
const LOCAL_ITEM_RARITY_PREFIX = {
  common:    ['낡은','투박한','평범한','가벼운','흔한','오래된','소박한','거친','닳아빠진'],
  uncommon:  ['정교한','단련된','예리한','견고한','잘 벼려진','다듬어진','매끄러운','묵직한','세공된'],
  rare:      ['빛나는','마력이 깃든','서리 맺힌','불꽃을 두른','번개를 머금은','독기가 서린','그림자에 물든','별빛이 감도는','얼어붙은'],
  epic:      ['심연에서 벼려진','영혼이 깃든','폭풍을 머금은','운명을 뒤튼','파멸을 부르는','금기의','신벌이 깃든','천공을 가르는'],
  legendary: ['태초의','신화 속','별빛의','세상을 가른','종말을 고하는','신들이 남긴','시간을 초월한','창세의','만물을 꿰뚫는'],
};
const LOCAL_ITEM_ORIGIN_BANK = [
  '멸망한 왕국의','잊혀진 마법사의','고대 용의','태고의 정령의','전설 속 기사의','심연에서 건져올린',
  '별을 삼킨 마수의','폐허가 된 신전의','이름 없는 대장장이의','황혼의 군주의','저주받은 무덤의',
  '전쟁으로 사라진 나라의','바다 밑 유적의','불타버린 도서관의','얼음 왕좌의','사막에 묻힌 신탁의',
  '천 년을 잠든 거인의','금지된 의식의',
];
...
const LOCAL_ITEM_DESC_BANK = {
  common:    ['어디서나 볼 수 있는 흔한 물건이다.','오래 써서 손에 익은 도구다.','투박하지만 제 몫은 한다.',
              '값은 싸지만 제 기능은 다 한다.','시장 어디서나 쉽게 구할 수 있다.','수수하지만 나쁘지 않은 만듦새다.',
              '초심자에게 어울리는 물건이다.','특별할 것 없는 평범한 물건이다.'],
  uncommon:  ['제법 손이 많이 간 물건이다.','전문 장인의 손길이 느껴진다.','평범한 물건보다는 한 수 위다.',
              '기본기에 충실한 좋은 물건이다.','숙련된 장인의 흔적이 엿보인다.','값어치를 하는 튼튼한 물건이다.',
              '모험가들 사이에서 평판이 좋다.','적당한 값에 적당한 성능을 낸다.'],
  rare:      ['희귀한 재료로 만들어져 독특한 기운이 감돈다.','평범한 물건과는 격이 다르다.','손에 쥐는 순간 다른 무게감이 느껴진다.',
              '쉽게 구할 수 없는 진귀한 물건이다.','다루는 이의 실력을 한층 끌어올려준다.','내력을 아는 상인이라면 눈을 크게 뜰 물건이다.',
              '희소성만큼이나 값어치도 상당하다.','손에 익히면 놓을 수 없게 되는 물건이다.'],
  epic:      ['보는 것만으로도 위압감이 느껴진다.','아무나 다룰 수 없는 물건이다.','범상치 않은 기운이 은은하게 감돈다.',
              '한 지역의 역사를 바꿀 만한 물건이다.','소유주에게 막대한 부담과 힘을 동시에 안긴다.','수집가라면 무엇과도 바꾸지 않을 물건이다.',
              '전장에서 이 물건 하나가 승패를 갈랐다는 이야기가 있다.','평범한 인간이 다루기엔 벅찬 힘이 서려 있다.'],
  legendary: ['전설로만 전해지던 물건이 눈앞에 있다.','시대를 초월한 존재감을 뿜어낸다.','역사에 이름을 남긴 물건임이 분명하다.',
              '이 물건 하나로 왕국의 운명이 갈렸다는 전설이 있다.','오직 선택받은 자만이 온전히 다룰 수 있다고 전해진다.','박물관에서도 진품 여부를 논쟁할 만큼 희소하다.',
              '세상에 단 하나뿐인 물건일지도 모른다.','신화와 역사의 경계에 서 있는 물건이다.'],
};
const LOCAL_ITEM_LORE_OPEN = [
  ...
  '대장간 한구석에서 우연히 발견됐다는 소문이 있다.',
  '정확한 제작 시기조차 알려지지 않았다.',
  '옛 상인의 장부 한켠에 짧게 기록되어 있을 뿐이다.',
  '폐허를 뒤지던 모험가의 손에 처음 들렸다고 한다.',
  '누가 처음 만들었는지는 아무도 확실히 모른다.',
];
const LOCAL_ITEM_LORE_EVENT = [
  ...
  '여러 전쟁터를 거치며 이름이 조금씩 알려졌다.',
  '한때 왕실의 보물창고에 보관되었다는 기록도 있다.',
  '도굴꾼들 사이에서 오랫동안 회자되었다.',
  '한 상인이 거금을 주고 사들였다가 다시 팔아넘겼다고 한다.',
  '잃어버렸다 되찾기를 몇 번이나 반복했다는 이야기가 전해진다.',
];
const LOCAL_ITEM_LORE_CLOSE = [
  ...
  '진짜 가치를 알아보는 이는 드물다.',
  '앞으로도 누군가의 손에서 새로운 이야기를 쓰게 될 것이다.',
  '그 무게만큼이나 사연도 만만치 않다.',
  '이 물건을 손에 쥔 이의 운명이 어떻게 흘러갈지는 아무도 모른다.',
  '오래된 물건일수록 말보다 무게로 이야기한다.',
];
