...
export function doInteraction(action, cost=0, transportType='walk'){
  ...
  const actions = {
    ...
    // 편린」은 정적 드랍처가 코드 어디에도 없어 AI가 서사 중 지급해줘야만
    // 얻을 수 있었다(무-API 완주 원칙과 정면으로 어긋나는 진짜 공백 —
    // job/042의 conditionHint가 스스로 "도서관이나 유적에서 찾을 수
    // 있는"이라고 적어뒀는데, 정작 도서관/유적 조사 상호작용(바로 이
