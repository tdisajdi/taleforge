...
const LOCAL_FLAVOR_TEMPLATES = {
  '대화': [
    '{t}에게 다가가 지금 상황에 대해 조심스레 묻는다',
    '{t}의 표정을 살피며 속내를 가늠해본다',
    '{t}에게 넌지시 도움을 요청해본다',
    '일부러 다른 화제를 꺼내 {t}의 반응을 떠본다',
    '{t:과와} 눈을 맞추며 진심을 담아 이야기한다',
    '{t}에게 짧은 농담을 건네 분위기를 살핀다',
    '{t}의 말을 끝까지 들어보기로 한다',
    '{t}에게 솔직한 심정을 털어놓는다',
    '{t:을를} 설득할 만한 이유를 차분히 늘어놓는다',
    '{t:과와} 거리를 좁히며 대화를 이어간다',
  ],
  '실리': [
    '주변을 자세히 살펴 놓친 단서가 없는지 확인한다',
    '{t:을를} 꼼꼼히 살펴본다',
    '값나가 보이는 것이 있는지 훑어본다',
    '발밑과 그림자 진 구석까지 살핀다',
    '{t:과와} 흥정을 시도해본다',
    '실마리가 될 만한 것을 챙긴다',
    '지금 상황에서 가장 이득이 될 방법을 계산한다',
    '{t}의 정체나 용도를 가늠해본다',
    '주머니 사정을 다시 확인하며 다음 수를 고민한다',
    '돌아갈 길이나 다른 출구가 있는지 확인한다',
  ],
  '대담': [
    '위험을 무릅쓰고 {t}에게 정면으로 다가간다',
    '직감을 믿고 과감하게 움직인다',
    '규칙 따위 무시하고 자기 방식대로 밀어붙인다',
    '{t:을를} 향해 도발적인 한마디를 던진다',
    '물러서지 않고 끝까지 버텨보기로 한다',
    '위태로운 길인 걸 알면서도 그대로 나아간다',
    '{t}의 허를 찌를 기회를 노린다',
    '손해를 감수하고서라도 밀어붙인다',
    '잠시 망설이다 결국 무모한 선택을 한다',
    '{t}에게 도박을 걸어보기로 한다',
  ],
  '경계': [
    '만약을 대비해 무기에 손을 얹은 채 경계한다',
    '주변의 위협 요소를 재빨리 훑는다',
    '{t:로}부터 거리를 두며 상황을 지켜본다',
    '몸을 숨길 곳을 미리 봐둔다',
    '언제든 움직일 수 있도록 자세를 낮춘다',
    '숨을 고르며 다음 상황에 대비한다',
    '{t}의 움직임을 예의주시한다',
    '위험 신호가 없는지 온 신경을 곤두세운다',
    '만일의 상황에 대비해 퇴로를 확인해둔다',
    '긴장을 늦추지 않고 자세를 가다듬는다',
  ],
};
...
const TURN_REACT_BANK = {
  attack: {
    crit: ['{t}의 빈틈을 정확히 파고들어, 결정적인 일격을 꽂아 넣는다. 반격할 틈조차 주지 않는다.', '몸이 먼저 움직인다 — {t:은는} 미처 반응하지 못하고 치명적인 타격을 그대로 받아낸다.'],
    success: ['{t:을를} 향해 매섭게 파고들어 확실히 타격을 먹인다.', '노림수가 통했다. {t}에게 제대로 된 일격이 꽂힌다.'],
    fail: ['{t:이가} 한발 빨리 움직여, 공격이 허공을 가른다.', '거리 계산이 어긋나 {t}에게 제대로 닿지 않는다.'],
    critfail: ['균형을 잃고 크게 휘청인다 — {t}에게 오히려 빈틈을 내주고 만다.', '무리하게 힘을 준 탓에 자세가 무너지며, 위험한 틈이 생긴다.'],
  },
  defend: {
    crit: ['완벽한 타이밍으로 막아내며, {t}의 공세를 무력화시킨다.', '한 치의 오차도 없이 받아넘긴다 — {t:이가} 오히려 균형을 잃는다.'],
    success: ['제때 막아내며 피해를 최소화한다.', '{t}의 공격을 어렵지 않게 받아넘긴다.'],
    fail: ['막아섰지만 완전히 흘려내지 못해 충격이 전해진다.', '{t}의 힘에 밀려 자세가 무너진다.'],
    critfail: ['방어가 완전히 뚫린다 — {t}의 공격이 그대로 꽂힌다.', '엉뚱한 방향으로 몸을 틀어, 무방비 상태가 되고 만다.'],
  },
  fear: {
    crit: ['서슬 퍼런 기세에 {t:이가} 완전히 얼어붙는다.', '눈빛만으로 {t:을를} 제압한다 — 저항할 엄두조차 내지 못한다.'],
    success: ['위협이 먹힌다. {t}의 표정에 동요가 스친다.', '{t:이가} 한 걸음 물러서며 경계한다.'],
    fail: ['{t:은는} 별다른 동요 없이 받아넘긴다.', '위협이 크게 와닿지 않은 듯, {t}의 태도는 그대로다.'],
    critfail: ['오히려 {t:을를} 자극해, 상황이 더 험악해진다.', '위협이 역효과를 낸다 — {t}의 눈빛이 사나워진다.'],
  },
  stealth: {
    crit: ['그림자에 완전히 녹아든다 — {t:은는} 존재조차 눈치채지 못한다.', '숨소리 하나 흘리지 않고, 완벽하게 몸을 숨긴다.'],
    success: ['들키지 않고 자리를 잡는다.', '{t}의 시선을 피해 무사히 은신한다.'],
    fail: ['완전히 숨지 못해, {t}의 시선이 잠깐 스친다.', '몸을 숨기긴 했지만 어딘가 부자연스럽다.'],
    critfail: ['부스럭거리는 소리에 {t:이가} 곧바로 이쪽을 돌아본다.', '숨을 곳을 잘못 골라, 오히려 눈에 띄고 만다.'],
  },
  move: {
    crit: ['거침없이 내달려 순식간에 거리를 벌린다.', '몸놀림이 물 흐르듯 매끄럽다 — 단숨에 상황을 벗어난다.'],
    success: ['무사히 움직여 원하는 위치로 이동한다.', '큰 무리 없이 이동을 마친다.'],
    fail: ['발이 꼬여 속도가 붙지 않는다.', '생각만큼 빠르게 움직이지 못한다.'],
    critfail: ['발을 헛디뎌 크게 휘청인다.', '지형을 잘못 읽어 엉뚱한 곳으로 향하고 만다.'],
  },
  magic: {
    crit: ['마력이 완벽하게 응축되어, 강력한 힘이 뿜어져 나온다.', '주문이 한 치의 오차도 없이 완성된다.'],
    success: ['마력이 무리 없이 흘러나와 원하는 효과를 낸다.', '주문이 무사히 발동한다.'],
    fail: ['마력이 흩어져 위력이 크게 줄어든다.', '집중이 흔들려 주문이 온전히 완성되지 않는다.'],
    critfail: ['마력이 역류해 반동이 몸을 덮친다.', '주문이 완전히 어긋나며, 예상 못한 부작용이 인다.'],
  },
  persuade: {
    crit: ['말 한마디 한마디가 정확히 급소를 찌른다 — {t}의 마음이 완전히 돌아선다.', '논리와 진심이 함께 통했다. {t:이가} 순순히 고개를 끄덕인다.'],
    success: ['설득이 통한다. {t:이가} 조금씩 마음을 연다.', '{t:이가} 이야기를 진지하게 받아들인다.'],
    fail: ['{t:은는} 여전히 완고하게 버틴다.', '말이 제대로 와닿지 않은 듯, {t}의 표정이 굳어 있다.'],
    critfail: ['말실수가 {t}의 심기를 건드린다.', '오히려 {t}의 불신만 키우고 만다.'],
  },
  search: {
    crit: ['숨겨져 있던 단서가 한눈에 들어온다 — 놓칠 뻔한 것까지 전부 짚어낸다.', '예리한 관찰 끝에, 결정적인 흔적을 찾아낸다.'],
    success: ['눈에 띄지 않던 것을 발견한다.', '차분히 살핀 끝에 쓸 만한 단서를 얻는다.'],
    fail: ['별다른 소득 없이 시간만 흐른다.', '눈에 띄는 건 찾지 못한다.'],
    critfail: ['엉뚱한 곳을 뒤지느라 정작 중요한 걸 놓친다.', '서두르다 흔적을 훼손하고 만다.'],
  },
  social: {
    crit: ['대화가 예상보다 훨씬 부드럽게 흘러간다 — {t:이가} 마음을 활짝 연다.', '분위기를 완벽하게 읽어내며, {t:과와} 거리가 확 가까워진다.'],
    success: ['자연스럽게 대화가 이어진다.', '{t:과와} 무난하게 말을 주고받는다.'],
    fail: ['대화가 어색하게 겉돈다.', '분위기를 제대로 못 살려, {t}의 반응이 미적지근하다.'],
    critfail: ['말실수로 분위기가 싸늘해진다.', '엉뚱한 타이밍에 끼어들어 {t:을를} 당황시킨다.'],
  },
  resolve: {
    crit: ['생각한 대로, 아니 그 이상으로 상황이 풀린다.', '완벽하게 마음먹은 대로 흘러간다.'],
    success: ['별 무리 없이 원하는 대로 진행된다.', '순조롭게 상황이 정리된다.'],
    fail: ['생각만큼 쉽게 풀리지 않는다.', '뜻대로 되지 않아 잠시 멈칫한다.'],
    critfail: ['생각지도 못한 방향으로 어긋나 버린다.', '최악의 타이밍에 일이 꼬인다.'],
  },
};
