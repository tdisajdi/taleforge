...
// 코드는 손대지 않는다.
const LOC_GEN_NAME_PREFIX = ['잊혀진','안개 낀','붉은','서리 내린','고요한','그림자 진','낡은','바람 부는','달빛 어린','메마른','이끼 낀','폐허가 된','황폐한','저주받은','빛바랜','피에 물든','새벽의','황금빛','칠흑 같은','버려진','스산한','메아리치는','불타버린','얼어붙은','흙빛','희미한','외딴','잃어버린','침묵하는','오래된'];
const LOC_GEN_NAME_CORE = {
  dungeon: ['유적','폐허','지하 묘지','고대 신전','버려진 광산','저주받은 성채','지하 감옥','망자의 회랑','비밀 통로','잊힌 서고','바위굴','침수된 신전'],
  wilderness: ['숲','황무지','협곡','늪지','고원','동굴 지대','초원','절벽','습지','사막 지대','빙원','정글'],
  shrine: ['성소','제단','신단','기도처','사당','성지','묘당','봉헌소'],
  event: ['야영지','임시 주둔지','버려진 진지','전장의 흔적','야시장','임시 피난처'],
  special: ['균열','비경','봉인된 자리','차원의 틈','이계의 문','고대의 흔적'],
  village: ['마을','촌락','부락','정착지'],
  town: ['소읍','장터거리','시장 마을','교역소'],
  city: ['거리','구역','상업지구'],
  capital: ['왕성 외곽','궁성 부근','왕도 외곽'],
  hamlet: ['작은 정착지','외딴 촌'],
  port: ['포구','부두','항만'],
};
...
const LOC_GEN_DESC_BANK = {
  dungeon: ['오랜 세월 방치된 흔적이 곳곳에 남아있다. 안쪽에서 서늘한 공기가 흘러나온다.', '무너진 돌더미와 깨진 조각상들이 한때의 규모를 짐작케 한다.', '벽을 타고 흐르는 물소리가 유독 크게 울린다.', '군데군데 무너진 통로가 어디로 이어지는지 짐작하기 어렵다.', '오래된 먼지 냄새와 함께 서늘한 기운이 감돈다.', '발밑에서 부서진 잔해가 서걱거린다.'],
  wilderness: ['사람의 발길이 뜸한 듯, 자연 그대로의 거친 풍경이 펼쳐진다.', '바람과 풀냄새가 뒤섞여 낯선 정취를 자아낸다.', '멀리서 짐승 울음소리가 간간이 들려온다.', '풀숲 사이로 작은 오솔길이 희미하게 이어져 있다.', '땅거미가 내려앉으며 주변이 한층 스산해진다.', '이끼와 덤불이 뒤엉켜 길을 가늠하기 힘들다.'],
  shrine: ['공기가 유독 조용하고 무겁다. 오래된 신앙의 흔적이 느껴진다.', '누군가 여전히 이곳을 찾아와 기도를 올리는 듯하다.', '희미한 향내가 은은하게 감돈다.', '오래된 석상이 말없이 이곳을 지키고 있다.', '바람 한 점 없는데도 옷깃이 스치는 듯한 기척이 느껴진다.', '낡은 제구들이 가지런히 놓여 있다.'],
  event: ['최근까지 누군가 머물렀던 흔적이 남아있다.', '급하게 자리를 뜬 듯 어수선한 잔해가 흩어져 있다.', '꺼진 모닥불 자리에서 아직 온기가 남아있는 듯하다.', '흩어진 짐과 발자국이 다급했던 상황을 짐작케 한다.', '누군가 남긴 표식이 벽 한켠에 새겨져 있다.', '주변 풀이 짓눌린 흔적이 여럿 보인다.'],
  special: ['이 근방과는 확연히 다른 기운이 감돈다.', '설명하기 어려운 이질감이 감도는 곳이다.', '공기가 미묘하게 일그러져 보인다.', '가까이 다가설수록 묘한 긴장감이 든다.', '주변 소리가 이상하리만치 멀게 느껴진다.', '발을 딛는 순간 등줄기가 서늘해진다.'],
  village: ['소박하지만 사람 사는 온기가 느껴지는 곳이다.', '작은 집들이 옹기종기 모여 있다.', '마당에 널린 빨래와 아이들 웃음소리가 정겹다.', '흙길을 따라 소소한 살림살이가 늘어서 있다.', '해질 무렵 굴뚝마다 저녁 짓는 연기가 피어오른다.', '작은 우물가에 마을 사람들이 모여 있다.'],
  town: ['오가는 사람들로 제법 활기가 있다.', '장터의 소음과 냄새가 뒤섞여 있다.', '상인들의 호객 소리가 거리에 가득하다.', '여러 갈래 길이 이곳을 중심으로 뻗어 있다.', '지나가는 행상들의 수레바퀴 소리가 요란하다.', '작은 광장에 사람들이 삼삼오오 모여 있다.'],
  city: ['크고 복잡한 거리에 인파가 끊이지 않는다.', '높은 건물들 사이로 다양한 사람들이 오간다.', '골목마다 저마다의 사연을 품은 듯 분주하다.', '노점과 상점이 빼곡히 늘어서 있다.', '마차와 인파가 뒤섞여 거리가 복작인다.', '건물 사이로 낯선 언어들이 뒤섞여 들려온다.'],
  capital: ['위엄 있는 건축물들이 늘어서 있다.', '왕국의 중심다운 위압감이 느껴진다.', '깃발과 문장이 곳곳에 걸려 있다.', '경비의 발걸음 소리가 절도 있게 울린다.', '화려한 장식들이 왕국의 위상을 드러낸다.', '오가는 이들의 옷차림에서도 격식이 느껴진다.'],
  hamlet: ['작고 조용한 정착지다.', '몇 안 되는 집들이 서로 의지하듯 모여 있다.', '개짖는 소리 외엔 고요함만 감돈다.', '작은 텃밭들이 집 앞마다 자리하고 있다.', '마을 전체가 한눈에 들어올 만큼 아담하다.', '드문드문 피어오르는 연기만이 사람의 흔적을 알린다.'],
  port: ['짠내 나는 바닷바람과 함께 배들이 드나든다.', '선원과 상인들의 소리로 북적인다.', '그물과 밧줄이 부둣가에 어지러이 널려 있다.', '갈매기 울음소리와 파도 소리가 뒤섞인다.', '갓 들어온 배에서 짐을 부리는 소리가 요란하다.', '먼 바다 냄새가 옅게 배어 있는 골목이다.'],
};
const LOC_GEN_LORE_BANK = {
  dungeon: ['한때 번성했으나 알 수 없는 이유로 버려졌다는 이야기가 전해진다.', '이곳에서 사라진 이들의 이야기가 아직도 떠돈다.', '보물을 노리고 들어간 이들 중 돌아온 자가 드물다는 소문이 있다.', '오래전 이 일대를 지배했던 세력의 흔적이 남아있다는 설이 있다.', '깊은 곳엔 아직 밝혀지지 않은 무언가가 잠들어 있다고 전해진다.'],
  wilderness: ['오래전부터 이 일대의 사냥꾼과 채집꾼들이 드나들던 곳이다.', '계절마다 다른 짐승들이 이곳을 지나간다고 한다.', '길 잃은 여행자들이 종종 이곳에서 발견되곤 한다.', '토착민들 사이에서 조심해야 할 구역으로 알려져 있다.', '오래된 지도에도 표시되어 있지 않은 외진 곳이다.'],
  shrine: ['이 땅의 오래된 신앙과 관련이 있다고 전해진다.', '먼 옛날 이곳에서 기적이 일어났다는 전설이 있다.', '순례자들이 대대로 찾아오는 곳이라고 한다.', '이곳에서 올린 기도는 유독 잘 이루어진다는 소문이 있다.', '누가 이곳을 처음 세웠는지는 기록에도 남아있지 않다.'],
  event: ['최근 이 근방에서 일어난 소동의 흔적으로 추정된다.', '무슨 일이 있었는지 정확히 아는 이가 없다.', '근처 마을 사람들도 이곳을 꺼리기 시작했다는 이야기가 있다.', '누군가 급히 이곳을 떠난 흔적이 역력하다.', '이 소동과 관련된 소문이 인근에 퍼지고 있다.'],
  special: ['학자들 사이에서도 정체가 분분한 장소다.', '이곳을 연구하러 온 이들이 몇 있었지만 결론을 내지 못했다고 한다.', '평범한 자연 법칙으로는 설명되지 않는 현상이 목격됐다는 소문이 있다.', '오래된 문헌 몇 곳에서만 단편적으로 언급되는 장소다.', '가까이 다가가기를 꺼리는 이들이 적지 않다.'],
  village: ['조상 대대로 이 자리를 지켜온 이들이 산다.', '외부인에게는 낯설지만 정이 많은 사람들이 모여 산다.', '작은 축제나 풍습이 아직도 이어지고 있다고 한다.', '이곳 특산물로 인근에 이름이 조금 알려져 있다.', '오래된 우물 하나에 얽힌 이야기가 전해 내려온다.'],
  town: ['교역로가 이어지며 자연스레 커진 마을이다.', '여러 지역의 물건이 모이는 작은 거점이라고 한다.', '한때 이곳에서 큰 거래가 성사됐다는 이야기가 전해진다.', '상인들 사이에서 평판이 나쁘지 않은 곳이다.', '지나가는 이들이 잠시 쉬어가기 좋은 곳으로 알려져 있다.'],
  city: ['오랜 역사를 가진 지역 중심지다.', '여러 세력의 흥망을 지켜봐 온 곳이라고 한다.', '이곳 출신의 유명 인사가 여럿 있다는 이야기가 있다.', '번영과 쇠퇴를 여러 차례 겪었다는 기록이 남아있다.', '다양한 문화가 뒤섞여 독특한 분위기를 자아낸다.'],
  capital: ['왕국의 역사와 함께해온 상징적인 장소다.', '역대 통치자들의 발자취가 곳곳에 남아있다.', '중요한 결정들이 이곳에서 여러 차례 내려졌다고 한다.', '왕국의 흥망을 가장 가까이에서 지켜본 곳이다.', '이곳의 돌 하나하나에도 사연이 있다는 말이 있다.'],
  hamlet: ['외지고 조용해 방문객이 드물다.', '외부와의 교류가 적어 독자적인 풍습이 남아있다고 한다.', '이곳 주민들은 낯선 이를 경계하면서도 은근히 살갑다.', '작지만 나름의 자급자족 방식을 오래 이어왔다.', '지도에도 잘 표시되지 않는 외딴 곳이다.'],
  port: ['바닷길을 오가는 이들의 중요한 거점이다.', '먼 곳에서 온 상인과 선원들의 이야기가 넘쳐나는 곳이다.', '거친 파도와 함께 살아가는 사람들의 터전이다.', '이따금 낯선 나라의 물건이 흘러들어오곤 한다.', '오래전 난파선 이야기가 아직도 전해진다.'],
};
const LOC_GEN_INTERACTION_BANK = [
  { name:'주변 살피기', icon:'👁️', desc:'이 장소를 자세히 둘러본다.', action:'scavenge' },
  { name:'사람들과 대화', icon:'💬', desc:'근처 사람들에게서 소문을 듣는다.', action:'gatherGossip' },
  { name:'휴식 취하기', icon:'🛌', desc:'잠시 이곳에서 몸을 쉰다.', action:'innRest' },
  { name:'흔적 조사하기', icon:'🔍', desc:'이곳에 남은 흔적을 조사한다.', action:'investigateRuins' },
  { name:'조용히 명상하기', icon:'🧘', desc:'잠시 마음을 가다듬는다.', action:'meditate' },
  { name:'정보 수소문하기', icon:'📜', desc:'이곳에 대해 아는 이를 찾아본다.', action:'buyInfo' },
  { name:'기부하며 도움 청하기', icon:'✨', desc:'가진 것을 나누며 호의를 구한다.', action:'donate' },
  { name:'조용히 기도 올리기', icon:'🙏', desc:'이곳에서 잠시 마음을 기울인다.', action:'pray' },
];
...
export function composeLocalLocation(trigger, context){
  ...
  return {
    ...
    lore: pick(LOC_GEN_LORE_BANK[type]||LOC_GEN_LORE_BANK.village),
...
const NPC_QUEST_GREETING_BANK = {
  intimate: ['오, 자네였군! 마침 잘 됐네, 부탁할 게 하나 있는데.', '이게 누군가, 반갑구먼. 자네한테만 털어놓을 얘기가 있어.', '자네 얼굴 보니 마음이 놓이는구먼, 부탁 좀 들어주겠나.', '다른 누구도 아니고 자네니까 믿고 얘기하는 걸세.', '요즘 골치 아픈 일이 있는데, 자네 도움이 절실하네.', '언제 봐도 반갑구먼. 실은 부탁할 게 있어서 말일세.'],
  friendly: ['어이, 잠깐 시간 좀 내주겠소?', '마침 잘 왔소. 부탁할 게 하나 있는데.', '마침 잘 만났소, 긴히 할 얘기가 있는데.', '바쁘지 않으면 잠깐 이야기 좀 나눌 수 있겠소?', '당신이라면 믿고 맡길 만한 일이 있소.', '요즘 좀 곤란한 일이 있어서 말이오.'],
  neutral:  ['저... 잠깐 괜찮으시다면 드릴 말씀이.', '실례지만, 부탁 하나 드려도 될까요.', '바쁘신데 죄송합니다만, 잠시 시간 되실까요.', '이런 부탁 드려도 될지 모르겠습니다만.', '실례가 안 된다면 도움을 좀 청하고 싶습니다.', '급한 일이 있어서 여쭙습니다.'],
  distant:  ['...당신이라면 어쩔 수 없이 부탁해야겠군.', '탐탁지는 않지만, 달리 부탁할 사람이 없어서.', '내키진 않지만 별수 없이 부탁하는 거요.', '이런 일까지 당신께 청하게 될 줄은 몰랐소.', '다른 방법이 있었다면 이러지 않았을 거요.', '썩 내키진 않지만, 들어보시겠소.'],
  hostile:  ['흥, 당신 도움을 받고 싶진 않았는데.', '이런 부탁까지 하게 될 줄은 몰랐군, 솔직히.', '당신 도움 없이 해결됐으면 좋았겠지만.', '이 상황이 마음에 들지 않지만 방법이 없군.', '별수 없이 당신을 찾아왔소.', '기대는 안 하지만 일단 말이나 해보겠소.'],
};
const NPC_QUEST_OFFER_BANK = {
  intimate: ['자네니까 믿고 맡기는 걸세. 보상은 섭섭지 않게 두둑이 챙겨주지.', '이런 부탁 자네 말고 누구한테 하겠나. 사례는 확실히 하겠네.', '자네가 나서준다면 이보다 든든할 수 없지. 섭섭잖게 챙겨줌세.', '믿을 사람이 자네뿐이라 이리 부탁하네, 사례는 톡톡히 하지.', '이 일만 해결되면 발 뻗고 자겠네. 보답은 확실히 함세.', '자네라면 실수 없이 해낼 거라 믿네, 그만큼 챙겨주겠네.'],
  friendly: ['실은 도움이 필요한 일이 하나 있소. 보상은 섭섭지 않게 챙겨주겠소.', '이 일을 해결해주면 정말 큰 도움이 될 걸세. 대가는 확실히 치르겠네.', '어렵겠지만 당신 실력이라면 문제없을 거요, 보상은 넉넉히 하겠소.', '이 일 해결되면 나도 마음이 한결 가벼워질 것 같소.', '당신을 믿고 맡기는 거요, 대가는 후하게 쳐주겠소.', '부탁 하나 들어주면 나도 신세 하나 갚는 셈이오.'],
  neutral:  ['도와주신다면 정해진 대로 사례는 하겠습니다.', '부담 없이 들어주시면 감사하겠습니다. 보상은 준비해뒀습니다.', '정해진 보상은 확실히 드리겠습니다.', '약속드린 대로 사례는 틀림없이 하겠습니다.', '결과만 좋다면 보상에 인색하진 않겠습니다.', '수고에 걸맞은 대가를 준비해두겠습니다.'],
  distant:  ['제값은 쳐주겠지만, 그 이상은 기대 말게.', '어차피 거래니 손해는 안 보게 해주겠소.', '거래는 거래니 정확히 치르겠네.', '기대 이상은 바라지 말고, 정직하게 값은 치르겠소.', '괜한 소리 안 하고 정해진 만큼만 주겠소.', '서로 손해 볼 일 없게끔 처리하지.'],
  hostile:  ['어쩔 수 없이 부탁하는 거요. 보상은 약속대로 주겠소.', '이거 하나만 해결해주면 더 볼 일 없을 거요.', '약속한 건 지킬 테니 걱정 마시오.', '이 일 끝나면 더 볼 일도 없을 거요, 보상은 챙겨주겠소.', '마음에 안 들어도 값은 제대로 치르겠소.', '빚지고 싶지 않으니 보상은 확실히 주겠소.'],
};
